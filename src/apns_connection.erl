...
%% @hidden
open_feedback(Connection) when Connection#apns_connection.feedback_timeout > 0 ->
  case ssl:connect(
  ...
  case State#state.out_expires =< epoch() of
    ...
    false ->
      ...
      {error, Reason} ->
		...
		{noreply, State#state{out_expires = 0}}
    ...
    <<Command:1/unit:8, StatusCode:1/unit:8, MsgId:4/binary, Rest/binary>> ->
      case Command of
        8 -> %% Error
          ...
          try call(Error, [_MsgFailed#apns_msg.device_token, Status]) of
